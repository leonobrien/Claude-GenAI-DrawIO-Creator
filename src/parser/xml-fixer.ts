 ...
 */
function fixJsonEscaping(xml: string): { xml: string; applied: boolean } {
 ...
 */
function removeCdataWrapper(xml: string): { xml: string; applied: boolean } {
 ...
 */
function stripLlmArtifacts(xml: string): { xml: string; applied: boolean } {
  let fixed = xml;
 ...
 */
function escapeAmpersands(xml: string): { xml: string; applied: boolean } {
 ...
 */
function fixDoubleEscaped(xml: string): { xml: string; applied: boolean } {
  const fixed = xml
    .replace(/&amp;quot;/g, '&quot;')
    .replace(/&amp;lt;/g, '&lt;')
    .replace(/&amp;gt;/g, '&gt;')
 ...
 */
function fixCellTags(xml: string): { xml: string; applied: boolean } {
  const fixed = xml
 ...
 */
function fixMalformedClosingTags(xml: string): { xml: string; applied: boolean } {
 ...
 */
function fixMissingAttributeSpaces(xml: string): { xml: string; applied: boolean } {
  const fixed = xml.replace(/"(\w+)="/g, '" $1="');
  return { xml: fixed, applied: fixed !== xml };
}
 ...
 */
function removeLeadingGarbage(xml: string): { xml: string; applied: boolean } {
 ...
 */
function fixClosingTagTypos(xml: string): { xml: string; applied: boolean } {
  const fixed = xml
 ...
 */
function fixDuplicateIds(xml: string): { xml: string; applied: boolean } {
...
type FixStep = (xml: string) => { xml: string; applied: boolean };

const FIX_PIPELINE: Array<{ name: string; fix: FixStep }> = [
 ...
 */
export function fixXml(xml: string): FixResult {
  let current = xml;
